...
author:
  - STANISLAS Mélanie \texttt{first1.last1@xxxxx.com}
  - BILLY Maxime \texttt{first1.last1@xxxxx.com}
  - HOANG HUU Duc \texttt{first1.last1@xxxxx.com}
---
...
Our network topology consists of the following components (cf _Fig.X_):
...
This script automates everything from creating the network topology to launching the DDoS attack and reporting the results (cf _Fig.X_ [_Link_](https://raw.githubusercontent.com/ozeliurs/SDN-Security/main/papers/.project-files/ddos-attack/.assets/demo.gif)).
...
`h2` (`s1-eth2`) and `h4` (`s2-eth2`) pushing 80 Mbps (cf _Fig.X_ & _Fig.X_) each and `h5` (`s3-eth1`) pushing roughly 60 Mbps (cf _Fig.X_).
...
This network traffic is all directed towards the target server, as seen in the Sent/Received network traffic graph (cf _Fig.X_).
...
The traffic is distributed enough to saturate the 200 Mbps link to the server but not enough to saturate the 1 Gbps links between the switches (cf _Fig.X_ [_Link_](https://raw.githubusercontent.com/ozeliurs/SDN-Security/main/papers/.project-files/ddos-attack/.assets/bwm-ng/bwm_ng.gif)).
...
First, by analyzing traffic on `s3-eth2` (cf _Fig.X_), we can see the compromised hosts sending HTTP GET requests (in red) to the C2 server. The C2 server then responds with the command to execute (in blue).
...
Then, we can analyze the traffic on `s1-eth2` to see the SYN packets generated by the compromised hosts during the attack (cf _Fig.X_).
...
Finally, we can deduce how the attack unfolded (cf _Fig.X_).
